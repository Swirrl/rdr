...
// Component construct taken from OOK
const query = `
...
const render = (components) => {
  ...
  // remove components who are already included as children of another component
  components = components.filter(c => {
    return(!('subPropertyOf' in c))
  })
  // put components with more children first
  .sort((a,b) => {
    if ('children' in a) {
      if ('children' in b) {
  ...
  document.body.innerHTML = serialize(doc);
}