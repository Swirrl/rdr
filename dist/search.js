...
const dataset_query = (query2, limit = 10) => `
...
CONSTRUCT {
...
} WHERE {
  {
    SELECT DISTINCT ?code ?code_label_ WHERE {
      ...
      FILTER CONTAINS(?code_label_, '${query2}')
    }
    LIMIT ${limit}
  }
...
const query = () => dataset_query(get_query());
const fetch_triples = async (query_string, endpoint2) => {
  const triples = fetch(`${endpoint2}`, {
    method: "POST",
    headers: {
...
const search = () => {
  renderLoading();
  fetch_triples(query(), endpoint()).then(deserialise).then(renderResults);
};
const deserialise = async (text) => {
  const base = "http://gss-data.org.uk/";
  const jsonld = fromRDF(text, {format: "application/n-quads"}).then((jsonld2) => compact(jsonld2, context)).then((jsonld2) => append_expanded_uri(jsonld2, context, base)).then((jsonld2) => frame(jsonld2, framing));
  return jsonld;
};
const render = (hiccup) => {
  ...
  if (typeof graph !== "undefined") {
    const list = results["@graph"].map((dataset) => {
      return [
        ...
        dataset.components.map(({uri, label, codelist}) => {
          return [
            "div",
            {},
            [
            ...
            ["ul", {}, codelist.codes.map((c) => {
              return ["li.measure-wide", {}, resource_link(c.uri, `${c.label}`, "gray")];
            })]
          ];
        })
      ];
...
const resource_link = (uri, label, colour = "blue") => {
